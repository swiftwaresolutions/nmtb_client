    ...
    render: (value: any, record: any) =>
      value ? (
        <span
          ...
          onClick={(e) => {
            e.stopPropagation();
            (window as any).__viewSaleDetails?.(record);
          }}
        >
          {value}
        </span>
...
export default function PhSalesRegister() {
  const today = new Date().toISOString().split("T")[0];

  const [fromDate, setFromDate] = useState<string>(today);
  const [toDate, setToDate] = useState<string>(today);
  const [allRecords, setAllRecords] = useState<SaleRow[]>([]);
  const [displayedData, setDisplayedData] = useState<SaleRow[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<SaleRow | null>(null);

  // Expose view handler
  (window as any).__viewSaleDetails = (record: SaleRow) => {
  ...
  React.useEffect(() => {
    ...
    if (sortKey) {
      result = sortTableData(result, sortKey as keyof SaleRow, sortDirection);
    }
  ...
  const handleFilterSubmit = (e: React.FormEvent) => {
    ...
    setSubmitted(false);
    pharmacyApiService
      .fetchSalesDetails(fromDate, toDate, storeId)
      .then((response: SalesDetailsResponse[]) => {
        const mapped: SaleRow[] = response.map((item) => ({
      ...
      .catch(() => {
        showErrorToast("Failed to fetch sales data. Please try again.");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const handleExport = () => {
    const exportData = displayedData.map((r, i) => ({
      "S. No": i + 1,
      "Bill/Receipt No": r.billNo,
      "O .P Number": r.opNo,
      "Patient Name": r.patientName,
      Details: r.details,
      User: r.username,
      "Total (Rs.)": r.total.toFixed(2),
      "Discount (Rs.)": r.discount.toFixed(2),
      "Payabled (Rs.)": r.payable.toFixed(2),
      "Paid (Rs.)": r.paid.toFixed(2),
    }));
    exportToExcel(
      exportData,
      `Sales_Register_${formatReportDate(new Date(), "DD-MM-YYYY")}`,
      "Sales Register"
    );
  };
          ...
          <Card.Body style={{ padding: "0.75rem", display: "flex", flexDirection: "column" }}>
            {loading ? (
              <div className="text-center py-5 text-muted">
                ...
                <div className="mt-3">Loading sales register...</div>
              </div>
            ) : (
              <div
                style={{
      ...
      <Modal show={showDetailsModal} onHide={clearSelectedRecord} size="xl" centered>
        <Modal.Header closeButton>
          <Modal.Title style={{ fontWeight: "var(--font-weight-semibold)" }}>
            Bill Details {selectedRecord ? `- ${selectedRecord.billNo}` : ""}
          </Modal.Title>
        </Modal.Header>
