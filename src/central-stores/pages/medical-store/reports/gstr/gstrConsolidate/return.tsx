...
const Return: React.FC = () => {
  ...
  const [rows, setRows] = useState<ConsolidateSalesReturnRow[]>([]);
  ...
  const handleShow = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await apiService.fetchConsolidateSalesReturn(fromDate, toDate);
      setRows(res);
      setReportDates({ from: fromDate, to: toDate });
      setSubmitted(true);
    } catch {
      showErrorToast("Failed to fetch Consolidate Sales Return data.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(exportData, `Consolidate_Sales_Return_${fromDate}_to_${toDate}`);
  };

  const total = (key: keyof ConsolidateSalesReturnRow) =>
  ...
  return (
    <div style={{ padding: "1.25rem", height: "100%", overflowY: "auto" }}>
      <Card className="shadow-sm mb-3">
        ...
        >
          Consolidate Sales Return Report
        </Card.Header>
        <Card.Body>
        ...
        <Card className="shadow-sm">
          <Card.Body style={{ padding: "1rem" }}>
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              >
                CONSOLIDATE SALES RETURN REPORT ({formatDisplayDate(reportDates.from)} TO{" "}
