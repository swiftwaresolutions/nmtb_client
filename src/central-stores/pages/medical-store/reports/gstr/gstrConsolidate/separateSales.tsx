...
// Placeholder summary row type (to be wired to API later)
...
const SeparateSales: React.FC = () => {
  const [fromDate, setFromDate] = useState<string>(today);
  const [toDate, setToDate] = useState<string>(today);
  const [isLoading, setIsLoading] = useState(false);
  const [rows, setRows] = useState<ConsolidateGstSepSalesRow[]>([]);
  ...
  const handleShow = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await apiService.fetchConsolidateGstSepSales(fromDate, toDate);
      setRows(res);
      setReportDates({ from: fromDate, to: toDate });
      setSubmitted(true);
    } catch {
      showErrorToast("Failed to fetch Consolidate GST Separate Sales data.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    const exportData = rows.map((r) => ({
      ...
      "End OP": r.endOp,
      "OP Tot": r.opCount,
      "Total Bills": r.ipCount + r.opCount,
    ...
    exportToExcel(exportData, `Consolidate_GST_Sep_Sales_${fromDate}_to_${toDate}`);
  };

  const total = (key: keyof ConsolidateGstSepSalesRow) =>
    rows.reduce((s, r) => s + ((r[key] as number) ?? 0), 0);

  // Static placeholder summary rows — wire to API later
  ...
  return (
    <div style={{ padding: "1.25rem", height: "100%", overflowY: "auto" }}>
      <Card className="shadow-sm mb-3">
        ...
        >
          Consolidate GST Separate Sales Report
        </Card.Header>
        <Card.Body>
        ...
        <Card className="shadow-sm">
          <Card.Body style={{ padding: "1rem" }}>
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              >
                CONSOLIDATE GST SEPARATE SALES REPORT ({formatDisplayDate(reportDates.from)} TO{" "}
            ...
            <div style={{ overflowX: "auto" }}>
              <Table bordered size="sm" className="mb-0">
                <thead>
                  ...
                  {/* Row 2 */}
                  <tr>
                    <th style={thStyle}>Start IP</th>
                    <th style={thStyle}>End IP</th>
                    <th style={thStyle}>IP Tot</th>
                    <th style={thStyle}>Start OP</th>
                    <th style={thStyle}>End OP</th>
                    <th style={thStyle}>OP Tot</th>
                ...
                <tbody>
                  {rows.length === 0 ? (
                    ...
                    rows.map((row, idx) => (
                      <tr key={idx}>
                        ...
                        <td style={{ ...tdStyle, textAlign: "center" }}>{row.ipCount + row.opCount}</td>
                  ...
                  {rows.length > 0 && (
                    <tr style={totalRowStyle}>
                      ...
                      <td style={{ ...totalRowStyle, textAlign: "center" }}>{total("ipCount") + total("opCount")}</td>
