...
const fmt6 = (val?: number | null) => (val != null ? val.toFixed(6) : "0.000000");
...
const HsnSalesReturn: React.FC = () => {
  const [fromDate, setFromDate] = useState<string>(today);
  const [toDate, setToDate] = useState<string>(today);
  const [isLoading, setIsLoading] = useState(false);
  const [rows, setRows] = useState<HsnWiseSalesReturnRow[]>([]);
  ...
  const handleShow = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await apiService.fetchHsnWiseSalesReturn(fromDate, toDate);
      setRows(res);
      setReportDates({ from: fromDate, to: toDate });
      setSubmitted(true);
    } catch {
      showErrorToast("Failed to fetch HSN Wise Sales Return data.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(exportData, `HSN_Sales_Return_${fromDate}_to_${toDate}`);
  };

  const totalUnits  = rows.reduce((s, r) => s + (r.unit   ?? 0), 0);
  const totalNetAmt = rows.reduce((s, r) => s + (r.netAmt ?? 0), 0);
          ...
          <Card.Body style={{ padding: "1rem" }}>
            {rows.length === 0 ? (
              ...
              <>
                ...
                >
                  MEDICINE SALES RETURN DETAILS BETWEEN &quot;
                    ...
                    <tbody>
                      {rows.map((row, idx) => (
                        <tr key={idx}>
                          ...
                          <td style={{ ...tdStyle, textAlign: "right" }}>{fmt(row.unit)}</td>
                          <td style={{ ...tdStyle, textAlign: "right" }}>{fmt6(row.mrp)}</td>
                          <td style={{ ...tdStyle, textAlign: "right" }}>{fmt(row.netAmt)}</td>
                          <td style={{ ...tdStyle, textAlign: "right" }}>{fmt(row.tax)}</td>
                      ...
                      <tr style={totalRowStyle}>
                        ...
                        <td style={{ ...totalRowStyle, textAlign: "right" }}>{fmt(totalUnits)}</td>
                        <td style={totalRowStyle} />
                        <td style={{ ...totalRowStyle, textAlign: "right" }}>{fmt(totalNetAmt)}</td>
                        <td style={totalRowStyle} />
