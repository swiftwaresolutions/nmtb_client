...
const HsnSales: React.FC = () => {
  const [fromDate, setFromDate] = useState<string>(today);
  const [toDate, setToDate] = useState<string>(today);
  const [isLoading, setIsLoading] = useState(false);
  const [rows, setRows] = useState<HsnWiseSalesRow[]>([]);
  ...
  const handleShow = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await apiService.fetchHsnWiseSales(fromDate, toDate);
      setRows(res);
      setReportDates({ from: fromDate, to: toDate });
      setSubmitted(true);
    } catch {
      showErrorToast("Failed to fetch HSN Wise Sales data.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(exportData, `HSN_Sales_${fromDate}_to_${toDate}`);
  };
          ...
          <Card.Body style={{ padding: "1rem" }}>
            {rows.length === 0 ? (
              ...
              <>
                ...
                >
                  MEDICINE SALES DETAILS BETWEEN &quot;
                  ...
                  <Table bordered size="sm" className="mb-0">
                    <thead>
                      <tr>
                        ...
                        <th style={{ ...thStyle, textAlign: "right" }}>Net Price</th>
                    ...
                    <tbody>
                      {rows.map((row, idx) => (
                        <tr key={idx}>
                          ...
                          <td style={{ ...tdStyle, textAlign: "right" }}>{fmt6(row.netPrice)}</td>
