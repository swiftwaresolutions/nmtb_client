...
const BillWiseSalesReturn: React.FC = () => {
  ...
  const handleShow = async (e: React.FormEvent) => {
    ...
    try {
      const [res, sumRes] = await Promise.all([
        apiService.fetchBillWiseSalesReturn(fromDate, toDate),
        apiService.fetchSalesReturnSummary(fromDate, toDate),
      ]);
      setRows(res);
      ...
      showErrorToast("Failed to fetch Bill Wise Sales Return data.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {submitted && (
        <Card className="shadow-sm">
          <Card.Body style={{ padding: "1rem" }}>
            ...
            >
              SALES RETURN GST DATE BETWEEN ({formatDisplayDate(reportDates.from)} AND{" "}
...
export default BillWiseSalesReturn;
