...
const CulturePrint: React.FC<CulturePrintProps> = ({ patient, tests, onBack, finalBillId }) => {
  ...
  const [cultureResultsByTest, setCultureResultsByTest] = useState<Record<number, CultutreTestResultByRegIdResponse>>({});
  ...
  // Load culture results for all tests
  useEffect(() => {
    let mounted = true;
      ...
      try {
        const cultureEntries = await Promise.all(
          stableTests.map(async (test) => {
            const result = await labApiService.fetchCultureTestResultByTestRegId(test.testRegId);
        ...
        console.error("Error loading culture print data", error);
    ...
    <>
      <style>
        {`
          ...
          .culture-summary-row + .culture-summary-row {
                ...
                <td className="print-layout-cell">
                  <div className="print-repeating-header">
                    {(() => {
                      ...
                      const firstCultureResult = firstTest ? cultureResultsByTest[firstTest.testRegId] : null;
                      return (
                        <CulturePrintPatientInfo
                          opno={patient.opNumber}
                          finalBillId={finalBillId}
                          specimenName={firstTest?.specimenName}
                      ...
                      {tests.map((test, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* Blood Culture Organisms (nonRective) */}
                            {hasNonRective && (
                              <div style={{ marginBottom: "0.5rem" }}>
                                <div style={{ fontWeight: "var(--font-weight-bold)", marginBottom: "0.2rem" }}>
                                  Blood Culture for Entric and Non-Entric Organisms:
                                </div>
                                <div className="text-center" style={{ whiteSpace: "pre-wrap" }}>{cultureResult.nonRective}</div>
                              </div>
                            )}
                            ...
                            {hasAntibiogramData && (
                              <>
                                {/* ANTIBIOGRAM */}
                                <div style={{ textAlign: "center", fontWeight: "var(--font-weight-bold)", fontSize: "var(--font-size-base)", letterSpacing: "0.1em", margin: "0.75rem 0 0.5rem" }}>
                                  ANTIBIOGRAM
                                </div>
                                ...
                                <Table bordered size="sm" className="antibiogram-table">
                                  ...
                                  <tbody>
                                    {Array.from({ length: maxRows }).map((_, rowIdx) => (
                                      <tr key={rowIdx}>
                                        <td className="antibiogram-ant-cell">
                                          {sensitive[rowIdx]?.antName || ""}
                                        </td>
                                        <td className="antibiogram-zone-cell">
                                          {sensitive[rowIdx]?.value || ""}
                                        </td>
                                        <td className="antibiogram-ant-cell">
                                          {moderate[rowIdx]?.antName || ""}
                                        </td>
                                        <td className="antibiogram-zone-cell">
                                          {moderate[rowIdx]?.value || ""}
                                        </td>
                                        <td className="antibiogram-ant-cell">
                                          {resistant[rowIdx]?.antName || ""}
                                        </td>
                                        <td className="antibiogram-zone-cell">
                                          {resistant[rowIdx]?.value || ""}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
