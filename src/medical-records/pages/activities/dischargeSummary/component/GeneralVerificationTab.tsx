...
interface GeneralVerificationTabProps {
...
const GeneralVerificationTab: React.FC<GeneralVerificationTabProps> = ({ onPatientSelect, onPrintPatient, fromDate, toDate }) => {
  ...
  const fetchVerificationPatients = async () => {
    setLoading(true);
    try {
      const response = await apiService.fetchIpPatientsForSummaryVerify(fromDate, toDate);
      ...
      // Filter: show only patients where dischargeSummaryExists=1 AND dischargeSummaryVerified!=1
      const filteredPatients = Array.isArray(data)
        ? data.filter((patient: any) => 
      ...
      const verificationData: VerificationPatientData[] = filteredPatients.map((patient: any, index: number) => ({
        ...
        summaryUser: patient.summaryUser || "",
  ...
  const handleRowClick = async (record: VerificationPatientData) => {
    ...
    try {
      ...
      if (summaryData) {
        const enhancedRecord = { ...record, summaryData };
        if (onPatientSelect) onPatientSelect(enhancedRecord);
        showSuccessToast("Patient details loaded successfully");
      } else {
        showErrorToast("No discharge summary found for this patient");
        if (onPatientSelect) onPatientSelect(record);
      }
    } catch (error: any) {
      console.error("Error fetching general discharge summary:", error);
      showErrorToast(
  ...
  const handleApprove = async (record: VerificationPatientData) => {
    ...
    try {
      ...
      const response = await apiService.approveDischargeSummary({
        summaryId: summaryData.id,
        visitId: record.visitId,
        approveUid: loginData.id || 0
      });

      showSuccessToast(response?.message || "Discharge summary approved successfully!");
      fetchVerificationPatients();
    } catch (error: any) {
...
export default GeneralVerificationTab;
