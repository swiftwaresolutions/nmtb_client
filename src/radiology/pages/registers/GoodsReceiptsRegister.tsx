...
export default function GoodsReceiptsRegister() {
    // Navigation
    ...
    const updateDisplayedData = (records: any[], search: string, sortK: string, sortDir: "asc" | "desc") => {
        ...
        // Apply search
        if (search) {
            result = searchTableData(result, search, ["grnNo", "poNo", "supplierName", "supplierAddress", "invoiceNo"]);
        }
    ...
    const handleExport = () => {
        const exportData = displayedData.map((record, index) => {
            const profit = (record.totalMRP || 0) - (record.totalCost || 0);
            const profitPercent = record.totalCost > 0 ? ((profit / record.totalCost) * 100) : 0;
            return {
                "S.No": index + 1,
                "GR No": record.grnNo,
                "PO No": record.poNo,
                ...
                "Inv. Date": record.invoiceDate,
                "Date": record.receiptDate,
                "Total Value": record.totalCost,
                "MRP Value": record.totalMRP,
                "Profit": profit,
        ...
        exportToExcel(
            exportData,
            `Goods_Receipts_Register_${formatReportDate(new Date(), "DD-MM-YYYY")}`,
    ...
    const columns = [
        ...
        { key: "grnNo", label: "GR No", sortable: true },
        ...
        { key: "invoiceDate", label: "Inv. Date", sortable: true },
        { key: "receiptDate", label: "Date", sortable: true },
        { key: "totalCost", label: "Total Value", sortable: true },
        { key: "totalMRP", label: "MRP Value", sortable: true },
        { 
            key: "profit", 
            label: "Profit", 
            sortable: true,
