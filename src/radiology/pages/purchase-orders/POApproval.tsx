...
const POApproval: React.FC = () => {
    ...
    const handleApprove = (orderId: number) => {
        ...
        if (order) {
            const confirmed = window.confirm(
                `Approve Purchase Order ${order.orderNo}?\n\n` +
            ...
            if (confirmed) {
                ...
                alert(`Purchase Order ${order.orderNo} has been approved!`);
            }
        }
    };
    ...
    return (
        <Container fluid className="p-4">
            <PageHeader icon={faCheckSquare} title="P.O. Approval" subtitle="Review and approve purchase orders" />
                        ...
                        <Accordion.Item eventKey={`order-${order.id}`} key={order.id}>
                            <Accordion.Header>
                                <Row className="w-100 align-items-center">
                                    ...
                                    <Col md={4}>{order.supplierName || <span className="text-muted">-</span>}</Col>
                                    ...
                                    <Col md={1}>
                                        <Button
                                            variant={order.status === "approved" ? "success" : "primary"}
                                            size="sm"
