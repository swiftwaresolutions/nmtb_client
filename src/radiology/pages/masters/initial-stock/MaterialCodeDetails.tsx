...
const MaterialCodeDetails = () => {
    ...
    // State for input values (only addition, no action dropdown needed)
    const [additionValues, setAdditionValues] = useState<Record<number, string>>({});

    // Handle value input change
    const handleValueChange = (id: number, value: string) => {
        setAdditionValues(prev => ({
            ...prev,
            [id]: value
        }));
    };

    // Handle common submit for all additions
    const handleSubmitAll = () => {
        // Collect all additions
        const additions = stockData
            .filter(item => {
                const value = parseFloat(additionValues[item.id] || "0");
                return value > 0;
            })
            .map(item => ({
                batchNumber: item.batchNumber,
                value: parseFloat(additionValues[item.id]),
                currentStock: item.stock
            }));

        if (additions.length === 0) {
            alert("Please enter addition values for at least one batch.");
            return;
        }

        // Build confirmation message
        const message = `Add the following stock?\n\n${additions
            .map(add => `Batch ${add.batchNumber}: +${add.value}`)
            .join("\n")}`;

        if (window.confirm(message)) {
            // Here you would typically make an API call
            alert(`All stock additions submitted successfully!\nTotal batches updated: ${additions.length}`);
            // Reset all values
            setAdditionValues({});
        }
    };
    ...
    const handleBack = () => {
        navigate("/hims/radiology/masters/initial-stock");
    };
        ...
        return (
            <Container fluid className="p-4">
                <Alert variant="warning">
                    ...
                    <p>Please select a material from the Initial Stock page.</p>
                    ...
                    <Table striped bordered hover responsive>
                        <thead>
                            <tr>
                                ...
                                <th style={{ width: "150px" }}>Addition Value</th>
                            </tr>
                        </thead>
                        <tbody>
                            {stockData.length === 0 ? (
