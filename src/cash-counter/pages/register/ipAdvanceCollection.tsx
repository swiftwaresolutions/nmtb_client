...
interface IPAdvanceCollectionRow {
  patientName: string;
  opNo: string;
  billNo: string;
  total: number;
...
function IPAdvanceCollection() {
  ...
  const [filteredData, setFilteredData] = useState<IPAdvanceCollectionRow[]>([]);
  const [displayedData, setDisplayedData] = useState<IPAdvanceCollectionRow[]>([]);
  ...
  const fetchAdvanceCollection = async () => {
    if (!fromDate || !toDate) {
    ...
    setLoading(true);
    try {
      const response: AdvanceReceivedData[] = await cashCounterApi.fetchAdvanceReceived(
        fromDate,
        toDate
      );
      ...
      const transformedData: IPAdvanceCollectionRow[] = response.map((item) => ({
      ...
      showErrorToast(
        error?.response?.data?.error || "Failed to fetch advance collection data"
      );
      setFilteredData([]);
      setDataLoaded(false);
    } finally {
  ...
  const updateDisplayedData = useCallback(
    (
      records: IPAdvanceCollectionRow[],
      search: string,
      sortK: string,
      sortDir: "asc" | "desc"
    ) => {
      let result: IPAdvanceCollectionRow[] = records;
      ...
      if (sortK) {
        result = sortTableData(result, sortK as keyof IPAdvanceCollectionRow, sortDir);
      }
  ...
  const tableWithTotal = useMemo<IPAdvanceCollectionRow[]>(() => {
    return [
      ...displayedData,
      {
        patientName: "Total",
  ...
  const handleExport = () => {
    const exportData = displayedData.map((row: IPAdvanceCollectionRow, index: number) => ({
      "S.No": index + 1,
    ...
    exportToExcel(
      exportData,
      `IP_Advance_Collection_${formatReportDate(new Date(), "DD-MM-YYYY")}`,
  ...
  const columns = [
    {
      ...
      label: "S. No",
      sortable: false,
      render: (_: any, record: IPAdvanceCollectionRow, index: number) => (record.isTotal ? "" : index + 1)
    },
    {
      key: "patientName",
      label: "Patient Name",
      sortable: true,
      render: (value: string, record: IPAdvanceCollectionRow) => (record.isTotal ? <strong>{value}</strong> : value)
    },
    { key: "opNo", label: "OP No", sortable: true, render: (value: string, record: IPAdvanceCollectionRow) => (record.isTotal ? "" : value) },
    { key: "billNo", label: "Bill No", sortable: true, render: (value: string, record: IPAdvanceCollectionRow) => (record.isTotal ? "" : value) },
    {
      key: "total",
      label: "Total",
      sortable: true,
      render: (value: number, record: IPAdvanceCollectionRow) => renderAmountCell(value, record.isTotal)
    },
    {
      key: "cashPaid",
      label: "Cash Paid",
      sortable: true,
      render: (value: string, record: IPAdvanceCollectionRow) => (
  ...
  return (
    <React.Fragment>
      <Container fluid className="px-4 py-3">
        <ReportHeader
          title={`${organization?.name?.toUpperCase() || "HOSPITAL"} - IP Advance Collection`}
...
export default IPAdvanceCollection;