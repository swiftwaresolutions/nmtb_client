...
import CashCounterApiService, {
  LabCollectReportRow,
  LabReturnCollectReportRow,
  LabDueCollectReportRow,
...
const getReturnAmount = (row: LabReturnCollectReportRow) => row.paid;
...
const AccLabCollection: React.FC = () => {
  ...
  const [labReportData, setLabReportData] = useState<LabCollectReportRow[]>([]);
  const [labReturnData, setLabReturnData] = useState<LabReturnCollectReportRow[]>([]);
  const [dueCollectionData, setDueCollectionData] = useState<LabDueCollectReportRow[]>([]);

  const displayedLabReportData = useMemo(() => {
    const search = searchTerm.trim().toLowerCase();
    if (!search) {
      return labReportData;
    }

    return labReportData.filter((row) => {
      return (
  ...
  }, [labReportData, searchTerm]);

  const displayedLabReturnData = useMemo(() => {
    const search = searchTerm.trim().toLowerCase();
    if (!search) {
      return labReturnData;
    }

    return labReturnData.filter((row) => {
      ...
      return (
        ...
        includesSearchValue(row.dateTime, search) ||
        includesSearchValue(row.fullName, search) ||
        includesSearchValue(row.retDisplay, search) ||
        includesSearchValue(row.total.toFixed(2), search) ||
        includesSearchValue(row.totDisc.toFixed(2), search) ||
        includesSearchValue(row.pay.toFixed(2), search) ||
        includesSearchValue(row.balance.toFixed(2), search) ||
  ...
  const totalLabCash = displayedLabReportData.reduce((sum, row) => sum + row.cashPaid, 0);
  const totalLabSwipe = displayedLabReportData.reduce((sum, row) => sum + row.swpPaid, 0);
  ...
  const totalReturn = displayedLabReturnData.reduce((sum, row) => sum + getReturnAmount(row), 0);
  const totalLabCollection = totalLabCash + totalLabSwipe;
  const totalDueCollection = totalDueCash + totalDueSwipe;
  const totalCashCollection = totalLabCash + totalDueCash;
  const totalSwipeCollection = totalLabSwipe + totalDueSwipe;
  const totalQrCollection = 0;
  const netCollection = totalLabCollection + totalDueCollection - totalReturn;
  const totalLabTotal = displayedLabReportData.reduce((sum, row) => sum + row.total, 0);
  const totalLabDiscount = displayedLabReportData.reduce((sum, row) => sum + row.totDisc, 0);
  const totalLabPayable = displayedLabReportData.reduce((sum, row) => sum + row.pay, 0);
  const totalLabBalance = displayedLabReportData.reduce((sum, row) => sum + row.balance, 0);
  const totalReturnTotal = displayedLabReturnData.reduce((sum, row) => sum + row.total, 0);
  const totalReturnDiscount = displayedLabReturnData.reduce((sum, row) => sum + row.totDisc, 0);
  const totalReturnPayable = displayedLabReturnData.reduce((sum, row) => sum + row.pay, 0);
  const totalReturnBalance = displayedLabReturnData.reduce((sum, row) => sum + row.balance, 0);
  ...
  const handlePrint = useReactToPrint({
    ...
    pageStyle: `
      * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      ...
      .acc-lab-print-header { display: block !important; margin-bottom: 8px; }
      .acc-lab-print-title { text-align: center; font-weight: 700; margin: 4px 0 2px; font-size: 11px; }
      .acc-lab-print-subtitle { text-align: center; margin: 0 0 6px; font-size: 9px; }
      .acc-lab-screen-only { display: block !important; }
      .acc-lab-print-only { display: none !important; }
      .acc-lab-print-summary { display: none !important; }
      table { width: 100%; border-collapse: collapse !important; }
      table, th, td { border: 1px solid #000 !important; }
      .acc-lab-print-table th,
      .acc-lab-print-table td {
        font-size: 11px !important;
        padding: 1px 3px !important;
        line-height: 1.05 !important;
      }
      .acc-lab-print-table thead th {
        padding: 2px 3px !important;
      }
      .acc-lab-print-summary table {
        width: auto !important;
        min-width: 280px;
        margin-left: auto !important;
        border-collapse: collapse !important;
      }
      .acc-lab-print-summary td {
        border: 1px solid #000 !important;
        padding: 2px 8px !important;
        font-size: 9px !important;
        line-height: 1.15 !important;
      }
      .acc-lab-print-summary .print-summary-label {
      ...
      @media print {
        .acc-lab-screen-only { display: none !important; }
        .acc-lab-print-only { display: block !important; }
        .acc-lab-print-summary { display: block !important; margin-top: 10px; }
      }
    `,
  });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const [labReportResponse, labReturnResponse, dueCollectionResponse] = await Promise.all([
        cashCounterApi.fetchLabCollectionReport(fromDate, toDate),
        cashCounterApi.fetchLabReturnCollectionReport(fromDate, toDate),
        cashCounterApi.fetchLabDueCollectReport(fromDate, toDate),
      ]);

      setLabReportData(labReportResponse);
      setLabReturnData(labReturnResponse);
      ...
      setSubmitted(false);
      showErrorToast("Failed to fetch lab report data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    const exportData = [
      ...displayedLabReportData.map((row, index) => ({
        "Report Type": "Lab Collection",
        "S.No": index + 1,
      ...
      ...displayedLabReturnData.map((row, index) => ({
        "Report Type": "Lab Return Collection",
        "S.No": index + 1,
        "OP No": row.displayNumber,
        "Patient Name": row.patientName,
        Date: row.dateTime,
        User: row.fullName,
        "Return Display": row.retDisplay,
        Total: formatAmount(row.total),
        Discount: formatAmount(row.totDisc),
        Pay: formatAmount(row.pay),
        Balance: formatAmount(row.balance),
    ...
    exportToExcel(
      exportData,
      `Lab_Report_${formatReportDate(new Date(), "DD-MM-YYYY")}`,
      "Lab Report"
    );
  };

  const renderLabCollectionTable = () => (
    <div style={{ overflowX: "auto" }}>
      <Table striped bordered hover className="table-hims mb-0 acc-lab-print-table acc-lab-screen-only">
        <thead>
          <tr>
        ...
        <tbody>
          {displayedLabReportData.length === 0 ? (
            <tr>
          ...
          >
            ...
            <td className="text-end">{formatAmount(totalLabCash)}</td>
            <td className="text-end">{formatAmount(totalLabSwipe)}</td>
            <td />
          </tr>
        </tbody>
      </Table>
  ...
  const renderLabReturnTable = () => (
    <div style={{ overflowX: "auto" }}>
      <Table striped bordered hover className="table-hims mb-0 acc-lab-print-table acc-lab-screen-only">
        ...
        <tbody>
          {displayedLabReturnData.length === 0 ? (
            <tr>
  ...
  const renderLabCollectionPrintTable = () => (
    <div className="acc-lab-print-only" style={{ display: "none", overflowX: "auto" }}>
      <Table bordered className="mb-0 acc-lab-print-table">
        <thead>
          <tr>
        ...
        <tbody>
          ...
          <tr style={{ fontWeight: "var(--font-weight-semibold)", background: "var(--color-table-header)" }}>
            ...
            <td className="text-end">{formatAmount(totalLabTotal)}</td>
            <td className="text-end">{formatAmount(totalLabDiscount)}</td>
            <td className="text-end">{formatAmount(totalLabPayable)}</td>
            <td className="text-end">{formatAmount(totalLabCash)}</td>
            <td className="text-end">{formatAmount(totalLabSwipe)}</td>
            <td className="text-end">{formatAmount(totalLabBalance)}</td>
          </tr>
        </tbody>
      </Table>
    </div>
  ...
  const renderLabReturnPrintTable = () => (
    <div className="acc-lab-print-only" style={{ display: "none", overflowX: "auto" }}>
      <Table bordered className="mb-0 acc-lab-print-table">
        <thead>
          <tr>
  ...
  return (
    <Container fluid className="p-3 d-flex flex-column" style={{ minHeight: "100vh" }}>
      <ReportHeader
        title={`${organization?.name?.toUpperCase() || "HOSPITAL"} - Lab Report`}
      ...
      {submitted && (
        <div className="d-flex flex-column gap-3 flex-grow-1">
          <div ref={printRef}>
            <div className="acc-lab-print-header" style={{ display: "none" }}>
              <PrintHeaderReports organization={printOrganization} />
              <div className="acc-lab-print-title">Lab Report</div>
              <div className="acc-lab-print-subtitle">{getDateRangeText(fromDate, toDate)}</div>
            </div>
            ...
            <Card className="shadow-sm">
              ...
              <Card.Body className="p-0">
                {renderLabCollectionTable()}
                {renderLabCollectionPrintTable()}
              </Card.Body>
              <Card.Footer className="d-flex justify-content-end align-items-center py-2 acc-lab-screen-only">
                <span
                  style={{
                    fontSize: "var(--font-size-md)",
                    fontWeight: "var(--font-weight-semibold)",
                ...
                >
                  ...
                  <span style={{ color: "var(--color-primary)" }}>
                    ₹ {formatAmount(totalLabCollection)}
                  </span>
                </span>
              </Card.Footer>
            </Card>
            ...
            <Card className="shadow-sm mt-3">
              ...
              >
                Lab Return Collection
              </Card.Header>
              <Card.Body className="p-0">
                {renderLabReturnTable()}
                {renderLabReturnPrintTable()}
              </Card.Body>
              <Card.Footer className="d-flex justify-content-end align-items-center py-2 acc-lab-screen-only">
                <span
                  style={{
            ...
            <div className="acc-lab-print-summary" style={{ display: "none" }}>
              <table>
                <tbody>
                  <tr>
...
export default AccLabCollection;
