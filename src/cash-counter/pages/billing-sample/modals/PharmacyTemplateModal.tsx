...
interface PharmacyTemplateModalProps {
  show: boolean;
  onHide: () => void;
  templates: PharmacyTemplate[];
  onSelectTemplate?: (template: PharmacyTemplate) => void;
}

const PharmacyTemplateModal: React.FC<PharmacyTemplateModalProps> = ({
  show,
  onHide,
  templates,
  onSelectTemplate
  ...
  const handleTemplateClick = (template: PharmacyTemplate) => {
    if (onSelectTemplate) {
      onSelectTemplate(template);
    }
  };

  const filteredTemplates = templates.filter((template: any) => template.isValid === 1);

  return (
    <Modal show={show} onHide={onHide} size="lg" centered>
      <Modal.Header closeButton className="bg-light border-bottom">
        <Modal.Title>Pharmacy Templates</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export default PharmacyTemplateModal;
